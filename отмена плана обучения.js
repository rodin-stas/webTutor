...
function cancelTest(task, person_id, education_plan_id) {
    ...
    if (activeTest != undefined) {
        ...
        // finishTest = tools.active_test_learning_finish(activeTest.id)
        // finishTest.TopElem.state_id = 1;
        // finishTest.Save();
        task.result_object_id = null;
        task.active_learning_id = null;
        task.state_id = 0;
    ...
    if (eduPlan != undefined && collabTE != undefined) {
        ...
        if (curator == undefined) {
            ...
            if (curator != undefined) {
                ...
                Log("plan_canceling_log", "отсутствует куратор в параметрах агента");
            }
        } else {
            curator = curator.TopElem
        }
...
Log("plan_canceling_log", "Неожиданная ошибка 2" + err);
}