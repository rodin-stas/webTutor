...
ShowAlert=false; // писать сообщения в логи сервера при запуске на стороне сервера или показывать сообщения в модальных окнах при запуске на клиенте
//alert('uniCalendarEntryPersonAgent starts'); // сообщение о запуске Агента - отключил

//OBJECT_ID = идентификатор первого выбранного объекта (строка)
if (ShowAlert) alert(OBJECT_ID)
//OBJECTS_ID_STR = список идентификаторов выбранных объектов, разделенных ';'
if (ShowAlert) alert(OBJECTS_ID_STR)
...
// условие ниже выполнитеся, только если получены параметры сервера отправки сообщений (условие выше)
if (SmtpFound)
{
	try
	{
		...
		docEvent = OpenDoc( UrlFromDocID(Int(OBJECT_ID)));
		...
		if ( docEvent!= undefined )
		{
			...
			if (ShowAlert) alert("Event="+tools.object_to_text(teEvent,"json"));
			
			file_id=String(teEvent.id).slice(String(teEvent.id).length-6); // выводит 6 последних символов id мероприятия, используется для формирования письма-события в календарь, видимо, такой способ нумерации
			...
			{
				// _header_host+'event&object_id='+teEvent.id+'&doc_id='+_doc_id - это ссылка на мероприятие в разделе Календарь мероприятий. В частном случае, если вдруг такая ссылка не работает (например, если раздел Календарь мероприятий доступен не всем) - можно сделать её короткой, только на само мероприятие, без раздела календарь мероприятий: _header_host+'event&object_id='+teEvent.id
				text_str='Тема: "'+teEvent.name+'" <br/>'+_header_host+teEvent.id;
				text_str_eng = 'Theme: "'+teEvent.name+'" <br/>'+_header_host+_event.id;
			}
			else
			{
				// _header_host+'event&object_id='+teEvent.id+'&doc_id='+_doc_id - это ссылка на мероприятие в разделе Календарь мероприятий. В частном случае, если вдруг такая ссылка не работает (например, если раздел Календарь мероприятий доступен не всем) - можно сделать её короткой, только на само мероприятие, без раздела календарь мероприятий: _header_host+'event&object_id='+teEvent.id
				text_str='Мероприятие "'+teEvent.name+'" запланировано: '+_header_host+teEvent.id;
			...
			_personList=String(OBJECTS_ID_STR).split(';')
			if (ShowAlert) alert('ArrayCount(_list)='+ArrayCount(_list)); // выводим сообщение в лог о количестве таких элементов
			...
			for (iParticipantElem in _personList) // пройдём по участникам мероприятия 
			{
				...
				{
					if (Trim(_elem)==Trim(iParticipantElem))
					{
				...
				_IDPerson=XQuery("for $elem in event_results where $elem/is_assist=true() and $elem/event_id = " + teEvent.id  + " and  $elem/person_id = " + iParticipantElem  + " return $elem");
				...
				{
					_person = ArrayOptFirstElem(XQuery('for $obj in collaborators where $obj/id='+iParticipantElem+' return $obj')); // находим участника в каталоге пользователей
					...
					if (!_person.email.HasValue) break;
					...
					if (ShowAlert) alert("Event="+tools.object_to_text(NewElem,"json"));
					_arr_str=_arr_str+iParticipantElem+";"; // дописываем данного сотрудника в переменную с сотрудниками, кому отправлялось уведомление
			...
			_subject="Invitation: "+teEvent.name+" ("+_time_zone_start_date+")"; // Тема - приглашение имя_мероприятия (дата начала мероприятия)
	...
	<tr>
		...
		'
			...
			else
			{
				...
				placeid >0 ? placeTimezoneId=teEvent.place_id.ForeignElem.timezone_id : placeTimezoneId=' '
				placeTimezoneId >0 ? placeTimezone=teEvent.place_id.ForeignElem.timezone_id.ForeignElem.name : placeTimezone=' '
				...
				if(iRecipientElem.curLng == 'ru'){
					...
					html_text += 'To review the sstages of your assigned learning solutions, please follow this <a href="https://websoft.nlmk.com/_wt/'+target_educ_plan_doc.TopElem.id+'" target="_blank">link</a>.';
				}
				html_text += '<hr />\r\n';
...
'
				...
				*/
				...
				{
					j = 0; // число, вычетаемое из id мероприятия для формирования уникального кода уведомления. Используется ниже в X-WR-ALARMUID:'+StrHexInt(Int(teEvent.id)-1-i)+'
					for (_phase in _array_phases)
					{
						...
						_subject="Invitation: "+teEvent.name+" ("+_time_zone_phase_start_date+")"; // Тема - приглашение имя_мероприятия (дата начала ресурса)
							...
							{	
								...
								if(target_educ_plan_doc != undefined)
								{
									...
									if(target_program != undefined && tools_web.is_true(target_program.custom_elems.ObtainChildByKey('firstStageLaunch').value))
									{
										...
										info = {
											itemId: String(elem_educ_plan.id),
											days: (daysLeft == 3 ? daysLeft + ' дня' : daysLeft == 7 ? daysLeft + ' дней' : ''),
											entryStage: (entryStage != undefined ? String(entryStage.name) : ''),
											entryPrestage: (entryPrestage != null ? String(entryPrestage.name) : ''),
										}
									}
...
'
					...
					*/
					...
					j++; // увеличиваем счётчик для X-WR-ALARMUID:'+StrHexInt(Int(teEvent.id)-1-i)+'
					}
				}
			}
