function authenificate(Request, AuthLogin, AuthPassword){
	...
	if (gmError != '')
	{
		...
		switch (gmError)
		{
			//внутренняя ошибка при проверке логина/пароля
			case 'form error1':
			...
			//в ЛОГИНе нет инофрмации UCSPersonId
			case 'login error2':
...
function processOAuthResponse(Request){
	...
	try
	{
        ...
        if (OptInt(tokenResponse.RespCode) != 200) {
            ...
            logError('Ошибка получения токена - ' + responseBody.error + ' - ' + responseBody.error_description);
	...
	{
		logError('Ошибка получения токена - ' + err);
	...
	{
		logError('Ошибка получения данных пользователя - ' + err);
...
function getNotAuthetificatedUri(Request){
    ...
    try {
        ...
        if (Request.Session.GetOptProperty('target_url') == undefined) {
            Request.Session.SetProperty('target_url', Request.Url);
        }
        return ('/default');
    }
