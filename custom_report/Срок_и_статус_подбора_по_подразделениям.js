try{
	...
	if(subdivision_id != undefined) {
		str = " AND [t3].[id] = "+SqlLiteral(subdivision_id)
	}
	...
	SELECT	\n\
		...
		d0.data.value('(/request/workflow_fields/workflow_field[name=''subdiv_id'']/value)[1]', 'varchar(300)') AS [subdiv_id],\n\
	...
	FROM\n\
		...
		INNER JOIN [subdivisions] AS [t3] ON [t3].[id] = [t1].[subdiv_id]\n\
	...
	WHERE\n\
		[t1].[org_id] = '7116612032298310523'" + str
	);
